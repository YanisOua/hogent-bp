...
Digital agencies streven ernaar om kwalitatieve, werkende webapplicaties en prototypes zo efficiënt mogelijk op te leveren. In presales- en projectcontext verwachten klanten niet alleen wireframes, maar interactieve applicaties die de uiteindelijke oplossing realistisch benaderen. Om dit proces te versnellen, investeren veel organisaties in design systems en componentenbibliotheken die hergebruik en visuele consistentie ondersteunen \parencite{Godbolt2016}. Binnen dergelijke omgevingen rijst de vraag hoe het design-to-codeproces, en in het bijzonder de initiële projectopstart, verder geautomatiseerd kan worden.

Ballistix Digital ontwikkelt diverse B2B-web\-applicaties die gebouwd zijn op een \textbf{gestandaardiseerde basisarchitectuur}. Ongeveer de helft van de projecten maakt gebruik van deze gedeelde technische basis, inclusief een eigen React\-componentenbibliotheek en vastgelegde huisstijl. Hierdoor delen veel applicaties dezelfde bouwstenen, zoals headers, navigatie, tabellen en formuliercomponenten. Ondanks deze gedeelde basis vereist de initiële projectopstart nog aanzienlijke manuele inspanning. Voor elk nieuw project moeten ontwikkelaars opnieuw pagina's aanmaken, datatypes definiëren, tabellen en formulieren opzetten, navigatie-elementen configureren en toegangsrechten (\emph{allowed\_views}) bijwerken. Deze stappen worden bij elk project grotendeels herhaald, wat leidt tot een repetitief en tijdrovend proces.

Dit repetitieve karakter heeft meerdere nadelige gevolgen. Enerzijds neemt de doorlooptijd van nieuwe projecten toe, wat de flexibiliteit beperkt in presales- en implementatietrajecten. Anderzijds ontstaat er een risico op inconsistent gebruik van de huisstijl en componenten, zeker wanneer verschillende ontwikkelaars betrokken zijn of wanneer projecten snel moeten worden opgestart.

Tegelijkertijd toont bestaande literatuur aan dat herhalende UI-patronen en standaardstructuren in hoge mate geschikt zijn voor template-, configuratie- of modelgedreven generatie \parencite{Nikiforova2025,Nasiri2023,Ourai2025}. De logica is eenvoudig: als het manueel hergebruiken van componenten al efficiëntiewinst oplevert, zal het \emph{automatisch} genereren en configureren van deze componenten bij de opstart dit effect aanzienlijk versterken.

Tegen deze achtergrond staat in deze bachelorproef de volgende centrale onderzoeksvraag centraal:
\begin{quote}
\emph{Hoe kan de initiële opzet van frontend-applicaties op basis van een gestandaardiseerde basisarchitectuur binnen een digital agency worden geautomatiseerd, zodat de ontwikkeltijd bij projectopstart vermindert met behoud van huisstijlconsistentie?}
\end{quote}

Om deze centrale onderzoeksvraag te beantwoorden, worden volgende deelvragen geformuleerd:
\begin{enumerate}
  \item Hoe verloopt de huidige projectopstart voor applicaties die gebruikmaken van een gedeelde architectuur binnen een digital agency, en welke activiteiten zijn daarin het meest repetitief en tijdrovend?
  \item Welke kwaliteits- en consistentie-eisen gelden er binnen de organisatie voor nieuwe applicaties binnen deze standaard?
  \item Welke benaderingen voor de automatisering van design-to-code en frontend-projec\-topstart worden in de literatuur en praktijk beschreven, en in welke mate zijn deze toepasbaar op de specifieke context van de casus?
  \item Welke functionele en niet-functionele requirements moet een automatiseringsoplossing voor de projectopstart bij een digital agency minimaal vervullen?
  \item In welke mate vermindert een proof-of-con\-cept-oplossing de benodigde tijd voor projectopstart, en blijft de huisstijlconsistentie behouden, vergeleken met de huidige manuele werkwijze?
\end{enumerate}
...
\begin{itemize}
  \item de huidige projectopstart binnen de gestandaardiseerde architectuur van de digital agency te analyseren en de belangrijkste pijnpunten en repetitieve activiteiten in kaart te brengen;
  \item op basis van vakliteratuur en praktijkcases relevante benaderingen voor automatisering van design-to-code en projectopstart te identificeren;
  ...
  \item een proof-of-concept te ontwikkelen dat deze automatisering realiseert binnen de technology stack van de agency, waarbij specifiek gekeken wordt naar de meerwaarde van \textbf{AI-oplossingen} (zoals LLM's of MCP) ten opzichte van klassieke templates;
...
De doelgroep van dit onderzoek bestaat in de eerste plaats uit de ontwikkelaars en technische verantwoordelijken bij de casusorganisatie, maar de resultaten zijn ook relevant voor andere digital agencies die met gelijkaardige gedeelde architecturen en design systems werken. Naast een technisch prototype levert deze bachelorproef een onderbouwde analyse en concrete aanbevelingen op voor het verder optimaliseren van de projectopstart en de bijhorende ontwikkelprocessen.

In de volgende sectie wordt de stand van zaken rond design-to-code, model- en configuratiegedreven ontwikkeling en automatisering van frontend-projectopstart besproken. Vervolgens wordt de gehanteerde methodologie toegelicht en worden de verwachte resultaten en de meerwaarde van het onderzoek uiteengezet.
%==============================================================================
% State-of-the-art
%==============================================================================
\section{State-of-the-art}
Het gebruik van large language models (LLM's) en andere generatieve technieken voor codegeneratie is de laatste jaren sterk toegenomen. Meerdere studies beschrijven hoe LLM's broncode kunnen genereren op basis van natuurlijke taalprompts, van kleine codefragmenten tot volledige functies en projecten \parencite{Zan2023NL2Code,Jiang2024CodeGenSurvey}. Bistarelli et al.\ tonen in een recente literatuurstudie dat deze modellen in veel scenario's de productiviteit kunnen verhogen, maar dat de gegenereerde code vaak kwaliteitsproblemen vertoont en nog manueel review en correctie vereist \parencite{Bistarelli2025LLMCodeReview}.

Een deel van de literatuur richt zich op model- en configuratiegedreven ontwikkeling. Nikiforova et al.\ bespreken hoe modeltransformaties in de vroege fasen van IT-projecten worden gebruikt om artefacten automatisch te genereren, zoals datamodellen en gebruikersinterfaces \parencite{Nikiforova2025}. Deze modelgedreven aanpakken tonen aan dat repetitieve en structureel vergelijkbare taken goed te automatiseren zijn, mits de relevante domeinconcepten (zoals entiteiten, relaties en schermtypes) expliciet worden gemodelleerd. In de praktijk bestaan daarnaast configuratiegedreven werkmodellen voor frontendontwikkeling. Ourai beschrijft hoe configuratiegedreven frontendontwikkeling toelaat om views, formulieren en gedrag declaratief te definiëren, zodat de uiteindelijke gebruikersinterface grotendeels uit configuratiebestanden en sjablonen kan worden opgebouwd in plaats van handgeschreven code \parencite{Ourai2025}. Veelvoorkomende patronen, zoals tabellen, formulieren en navigatie, worden hierbij geparametriseerd met metadata over velden, datatypes en rechten, in plaats van telkens opnieuw geprogrammeerd. Dit sluit goed aan bij situaties waarin applicaties expliciet werken met concepten zoals pagina's, datatypes en toegangsrechten (\emph{allowed\_views}), zoals bij de applicaties in de Ballistix-huisstijl.

Specifiek voor gebruikersinterfaces is recent ook werk verschenen rond LLM-gebaseerde codegeneratie voor gebruikersinterfaces. Wu et al.\ tonen met UICoder aan dat LLM's in staat zijn om code voor gebruikersinterfaces te genereren, maar dat de initiële uitvoer vaak niet compileert of visueel afwijkt van het beoogde ontwerp \parencite{Wu2024UICoder}. Door de LLM te combineren met geautomatiseerde feedbacklussen (zoals compileercontroles en visuele evaluatie) kan de kwaliteit van de gegenereerde code echter aanzienlijk worden verbeterd. Dit bevestigt dat generatieve technieken in de praktijk niet volstaan als volledig autonome ``zwarte doos'', maar expliciet moeten worden gevoed met domeinkennis, kwaliteitscriteria en feedback over de gegenereerde uitvoer.

Parallel aan deze evoluties verschijnen in de praktijk verschillende AI-tools die op basis van prompts complete projecten, componenten of gebruikersinterfaces genereren. Zulke prompt-to-code- of prompt-to-app-oplossingen zijn handig om snel een eerste versie van boilerplate-code of projectstructuur op te zetten, maar sluiten vaak niet automatisch aan bij de architectuur, componentenbibliotheek en huisstijl van een specifiek bedrijf. Zonder extra instructies levert dit vaak generieke code op die onze eigen componenten negeert. Voor organisaties die al investeren in een consistente set componenten en design tokens, zoals Ballistix Digital, blijft er daardoor veel manueel werk over om de gegenereerde code in lijn te brengen met de interne standaarden.

Samengevat laat de literatuur drie relevante inzichten zien. Ten eerste tonen model- en configuratiegedreven benaderingen dat repetitieve en structureel vergelijkbare taken, zoals het aanmaken van standaardpagina's, tabellen en formulieren, bijzonder geschikt zijn voor automatisering \parencite{Nikiforova2025,Ourai2025}. Ten tweede blijkt uit onderzoek naar generatieve code- en gebruikersinterfacegeneratie dat LLM's weliswaar sterk zijn in het genereren van generieke code en boilerplate-code, maar dat zonder expliciete context en feedback de aansluiting met een concrete componentenbibliotheek en huisstijl vaak beperkt blijft \parencite{Zan2023NL2Code,Bistarelli2025LLMCodeReview,Wu2024UICoder}. Ten derde is er in de praktijk een duidelijke trend naar tools die configuratie, templates en generatieve technieken combineren. Deze bevindingen vormen het vertrekpunt voor deze bachelorproef, waarin wordt onderzocht hoe de bestaande huisstijl en componentenbibliotheek van Ballistix Digital expliciet kunnen worden ingezet om de initiële projectopstart van frontend-applicaties gedeeltelijk te automatiseren.
...
\begin{itemize}
  \item \textbf{Onderzoeksmethoden:}
        \begin{itemize}
          ...
          \item \emph{Semigestructureerde interviews} met twee betrokken ontwikkelaars en/of technische verantwoordelijken om de werkwijze, frictiepunten en impliciete kennis rond projectopstart te verzamelen.
        \end{itemize}
...
\begin{itemize}
  \item \textbf{Onderzoeksmethoden:}
        \begin{itemize}
          \item \emph{Gerichte literatuurstudie} op basis van wetenschappelijke publicaties en professionele vakliteratuur over design-to-code, configuratiegedreven ontwikkeling, templategeneratie en generatieve technieken voor code- en gebruikersinterfacegeneratie \parencite{Zan2023NL2Code,Nikiforova2025,Ourai2025,Bistarelli2025LLMCodeReview,Wu2024UICoder}.
          \item \emph{Synthese} omtrent de specifieke context van Ballistix Digital, met aandacht voor herbruikbare patronen.
        \end{itemize}
...
De derde fase omvat het ontwerp en de ontwikkeling van een proof-of-concept (PoC) dat de meest repetitieve stappen in de projectopstart automatiseert.
...
\begin{itemize}
  \item \textbf{Onderzoeksmethoden:}
        \begin{itemize}
          ...
          \item \emph{Iteratieve ontwikkeling:} De PoC wordt gebouwd binnen de bestaande tech stack van Ballistix Digital (TypeScript, Next.js).
        \end{itemize}
        ...
        \begin{itemize}
          \item \emph{Quasi-experimentele vergelijking} tussen twee scenario's:
                \begin{enumerate}
                  ...
                  \item \emph{Met PoC}: projectopstart waarbij gebruikgemaakt wordt van het ontwikkelde proof-of-concept.
                \end{enumerate}
          \item selectie van twee à drie representatieve casussen (bijvoorbeeld typische applicatietypes of klantscenario's) waarvoor de initiële opzet wordt uitgevoerd in beide scenario's.
        \end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Februari tot Maart}: implementatie van het proof-of-concept en eerste interne testen.
