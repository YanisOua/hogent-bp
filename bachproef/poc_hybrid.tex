...
Het focus van dit hoofdstuk ligt op de hybride workflow, aangezien dit de experimentele kern van de PoC vormt. We beschrijven hoe een TypeScript-gebaseerde MCP-server~\autocite{modelcontextprotocol2026} de interne componentenbibliotheek, design tokens en code-genererende tools ontsluit, zodat een LLM in Cursor Pro~\autocite{CursorProUsage2026} gestructureerde aanvragen kan doen en gevalideerde code kan ontvangen. Deze architectuur minimaliseert hallucinaties door templates als ``safety rails'' in te zetten en code-generatie stap-voor-stap via MCP-tools uit te voeren.
...
De opzet van dit hoofdstuk volgt de evolutie van de implementatie: eerst presenteren we de algemene architectuur van de MCP-server (Sectie~\ref{sec:poc-architectuur}), daarna de setup en konfiguratie (Sectie~\ref{sec:poc-setup}), vervolgens de kerntools voor code-generatie (Sectie~\ref{sec:poc-tools}), daarna de gestructureerde resources en prompts (Sectie~\ref{sec:poc-resources}), en tenslotte de integratie met Cursor Pro inclusief token-tracking (Sectie~\ref{sec:poc-cursor}).
...
De MCP-gebaseerde workflow voor de Ballistix-projectopstart bestaat uit vier interconnecteerde onderdelen: (1) een \texttt{ballistix-mcp} TypeScript-server die tools en resources ontsluit, (2) een template-project dat als basis voor nieuwe projecten wordt gekopieerd, (3) een set van documentatie-resources die code-generatie-regels vastleggen, en (4) een LLM-client in Cursor Pro dat via MCP-protocol tools aanroept en resources raadpleegt.
...
Cursor Pro roept hierop de volgende fasen uit:
...
\begin{enumerate}
  ...
  \item \textbf{Tool-uitvoering (iteratief):} Voor elke pagina/component roept Cursor achtereenvolgens MCP-tools aan:
    \begin{itemize}
...
De TypeScript-gebaseerde MCP-server~\autocite{MCPTypeScriptSDK2025} serves twee soorten providers:

\begin{itemize}
  \item \textbf{Tools} — imperatieve acties (code genereren, bestanden schrijven, commits uitvoeren).
  \item \textbf{Resources} — declaratieve gestructureerde context (markdown-dokumenten met regels en conventies).
\end{itemize}

De server registreert zich in de globale Cursor-configuratie (\texttt{\~/.cursor}) en wordt automatisch beschikbaar voor alle \texttt{/plan}- en \texttt{/prompt}-instructies. Bij iedere interactie controleert Cursor welke tools en resources relevant zijn en levert deze aan het LLM.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{|l|l|X|l|}
    ...
    Voert \texttt{yarn lint:fix} uit in projectdir, danproceed \texttt{git commit} met gegeven message. Output: commit SHA. &
...
Resources zijn markdown-documenten die Ballistix-regels en -conventies codificeren. Wanneer Cursor een plan opmaakt, leest het relevante resources in en bevat deze in de context voor het LLM.
...
\begin{itemize}
  ...
  \item Base konfiguratie: \texttt{next.config.js}, \texttt{tailwind.config.ts}, \texttt{tsconfig.json}, \texttt{.eslintrc}.
...
Het centrale inzicht van deze architectuur is dat volledig vrije codegeneratie (''Unrestricted''-scenario uit Hoofdstuk~\ref{ch:methodologie}) tot hallucinaties en hoge token-kosten leidt. In plaats daarvan worden drie lagen van structuur ingezet:
...
\begin{enumerate}
  ...
  \item \textbf{Templates (Basisllaag):} Ingesloten code-templates in tools zorgen dat output altijd conform Ballistix-APIs is (bijv. \texttt{useRead}, \texttt{DataTableSection}).
\end{enumerate}

Deze gelaagde aanpak zorgt ervoor dat:

\begin{itemize}
  \item Hallucinaties sterk gereduceerd worden (geen vrije fantasie buiten templateboundaries).
  \item Token-verbruik afneemt (parameter-injection is goedkoper dan volledige code-generatie).
  \item Output altijd valideerbaar en testbaar is (geen custom API-aanroepen, altijd standaard hooks/componenten).
\end{itemize}

De afweging: meer hardcoding (templates) tegenover minder hallucinatie en lager token-verbruik — een praktische keuze gegeven de budgetbeperkingen van dit onderzoek~\autocite{CursorTokenTracking2025}.

\section{\IfLanguageName{dutch}{MCP-server setup en konfiguratie}{MCP-Server Setup and Configuration}}
\label{sec:poc-setup}

\subsection{Architectuur van de server}

De \texttt{ballistix-mcp}-server is een Node.js-applicatie geschreven in TypeScript dat via het Model Context Protocol~\autocite{MCPTypeScriptSDK2025} tools, prompts en documentatie-resources aan Cursor Pro aanbiedt. De server draait via stdio-transport (standaardstroom) en wordt gestart via een eenvoudig Node-commando.

\subsubsection{Entrypoint: server.ts}

De kerninitialisatie van de server gebeurt in \texttt{src/server.ts}:
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{|l|X|}
    ...
    Centrale konfiguratie van paden: \texttt{TEMPLATE\_PATH} (lokale ballistix-tool-template repo), \texttt{DOCS\_PATH} (mcp-docs). \\
    \hline
    \texttt{src/resources/} &
...
Wanneer de server actief is, luistert het naar stdin voor MCP-protocol-berichten en antwoordt via stdout. De server blijft in de achtergrond actief.

\subsection{Integratie met Cursor Pro}

\subsubsection{Cursor-configuratie}

De MCP-server wordt geregistreerd in Cursor's globale configuratie bestand \texttt{\~/.cursor/config.json}:
...
# Cursor CLI agent-modus gestart
  ...
  \item \textbf{Planning:} Het LLM analyseert de instructie en bepaalt welke tools aan te roepen (bijv. \texttt{create-ballistix-project}, \texttt{generate-overview-page}, \texttt{generate-filters}).
...
Cursor Pro~\autocite{CursorProUsage2026} biedt ingebouwde token-tracking via het \texttt{token}-paneel in het CLI-interface. Voor elke tool-aanroep en resource-raadpleging worden tokens gerapporteerd:

\begin{itemize}
  \item \textbf{Input tokens} — tokens voor de instructie, resources en context.
  \item \textbf{Output tokens} — tokens gegenereerd door het LLM.
...
De modelvergelijking gebeurt via dezelfde MCP-setup, zodat de enige variabele het LLM-model is. Dit stelt ons in staat effecten van model-choice op hallucinatie, token-verbruik en code-kwaliteit te isoleren (zie Evaluatie, Hoofdstuk~\ref{ch:resultaten}).
...
\begin{enumerate}
  ...
  \item \textbf{Evaluatie:} Token-tracking via Cursor Pro; model-vergelijking via dezelfde MCP-server met verschillende LLMs.
\end{enumerate}

Deze modulaire opzet zorgt ervoor dat wijzigingen aan tools, prompts of resources onmiddellijk beschikbaar zijn (na rebuild) voor alle Cursor-gebruikers, zonder dat de server handmatig opnieuw moet worden opgestart.
...
De \texttt{create-ballistix-project}-tool initaliseert een nieuw Ballistix-project door het template-project recursief te kopiëren naar een nieuwe projectmap, een verse git-repository aan te maken, en een initiële commit uit te voeren. Dit voorkomt dat de LLM de folder-structuur, package-configuratie en boilerplate zelf moet genereren.
...
export function registerCreateBallistixProjectTool(server: McpServer): void {
  server.registerTool(
    ...
    async ({ projectName, commitMessage }) => {
      ...
      // Stap 1: Kopieër template-project recursief
...
\begin{itemize}
  ...
  \item \textbf{Type-safety:} Zod-validatie van inputs erzorgt dat routeParam, entityTypeName valide TypeScript-identificators zijn.
...
Deze aanpak garandeert dat zelfs als de LLM \textit{wil} hallucineren over custom fetches of andere hooks, de template dit simpelweg niet toestaat.
...
\begin{itemize}
  \item \textbf{Expliciete constraints:} ``Filters nooit in primaryActions'' wordt herhaal om hallucinaties (``ik zet alles in primaryActions'') tegen te gaan.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{|l|l|l|}
    ...
    Markdown-dokumenten vastleggen gedragsregels &
...
Deze gelaagde aanpak zorgt ervoor dat hallucinaties langs meerdere wegen tegengegaan worden: zelfs als één laag faalt (bijv. LLM negeert prompt), de volgende laag (template-injectie in tool) biedt nog steeds bescherming.
...
Resources zijn niet-code-genererende MCP-providers die markdown-documentatie serveren als structured context. In tegenstelling tot tools (die imperatief code schrijven), bieden resources declaratieve, regelgebaseerde richtlijnen. Het LLM raadpleegt deze voortdurend bij het plannen van tool-aanroepen, wat hallucinaties minimaliseert door normen in te stellen voordat generatie begint.

\subsubsection{Resource-discovery en koppeling}

De MCP-server registreert zes documentatie-resources met unieke URI's (\texttt{ballistix-docs://rules/<slug>}). Hoe het LLM deze ontdekt:
...
\subsection{De zes dokumentatie-resources}
...
\begin{itemize}
  \item \textbf{Geen `*Overview`-naam:} Detail-pagina's krijgen een naam naar hun doel (bv. \texttt{CustomerInfo}, \texttt{DossierInfo}), niet \texttt{CustomerOverview}.
...
Naast resources registreert de MCP-server twee prompts — non-imperatieve richtlijnen die stap-voor-stap instructies geven.
...
\begin{itemize}
  ...
  \item Wijzig gén ander bestand behalve optioneel \texttt{*OverviewFilters}.
...
\begin{itemize}
  ...
  \item \textbf{Ingen eigenaardigheid:} Filters gaan in \texttt{secondaryActions} render-prop, niet als wrapper of toolbar-item.
  ...
  \item Wijzig gén andere bestanden.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{|l|l|X|}
    ...
    \hline
    Tools (code) &
    Tool-execution & \texttt{generate-overview} registreert scaffold-template met \texttt{secondaryActions} hook \\
    \hline
    Validatie (input) &
    Pre-tool & Zod-schema's valideren filter-spec-object; structuur erzwingt \texttt{secondaryActions}-mounting \\
    \hline
  \end{tabularx}
\end{table}

Zelfs als één laag faalt (bijv. agent negeert prompt), volgende laag biedt noch bescherming.
...
\begin{itemize}
  ...
  \item \textbf{Eigen \texttt{<table>} i.p.v. \texttt{TableList}:} Agent maakt custom HTML-tabel.
    \newline\textit{Preventie:} \texttt{table-list-rules} + context-injectie via resource forceert re-use.
...
Het template-project (\texttt{ballistix-tool-template}) is een pre-geconfigureerde, volledig werkende Next.js/React-applicatie die als \textit{copy-modify}-basis dient voor elk nieuw Ballistix-project. Het doel: verwijder de noodzaak om project-setup zelf te genereren; het LLM hoeft slechts incrementeel aan te passen.
...
\noindent
\textbf{Opmerking:} Template gebruikt **Formik + Yup** (niet Zod) voor formuliervalidatie.
...
Template gebruikt **Tailwind v4** met `@tailwindcss/postcss`:
...
\begin{itemize}
  ...
  \item \textbf{Geen} klassieke \texttt{.eslintrc} — flat config is standard.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Ondersteunde talen:} **Nederlands (nl)**, **Frans (fr)**, **Engels (en)** (resource desgewenst toevoegen).
...
Template bevat **geen lege placeholder-mappen**; \texttt{src/components/**} en \texttt{src/hooks/} bevatten al veel concrete modules:
...
Dit biedt het LLM concrete voorbeelden om van te erven; niet als lege scaffold.
...
\begin{itemize}
  ...
  \item \textbf{Locale-setup:} `locale.ts` defineert taallijst; init-resource is **nl** en **fr** (Engels desgewenst toevoegen).
\end{itemize}
...
\begin{itemize}
  \item \texttt{architecture.md} — Overzicht systeem-design.
...
Het template-project vormt de **onderste laag** van hallucinatie-preventie:

\begin{itemize}
  \item \textbf{Structuur gegeven:} Geen keuze over mappenindeling; LLM werkt within gegeven structuur.
...
De evaluatie meet de haalbaarheid en effectiviteit van de MCP-gebaseerde hybride architectuur via vier kernmetriekken: token-verbruik, hallucinatie-ratio, opstarttijd en code-kwaliteit. De setup vergelijkt drie LLM-modellen (Composer 2, Opus 4.7, GPT-5.5) over drie scenario's van toenemende complexiteit, met drie runs per combinatie voor stabiliteitsanalyse.

\subsection{Kernmetriekken}
...
\noindent
\textbf{Quantificering:} Per gegenereerde file: ``heeft specfout'' vs. ``OK''. Aggregate per project: aantal features met fouten / totaal features. \newline
...
\textbf{Doel:} Vaststellen of agent-gedrag stabiel is (verwacht: hoogstwaarschijnlijk zeer stabil dankzij deterministische template-injectie).
...
\noindent
\textit{Merk op:} Token-verbruik groeit sublineair met scenario-complexiteit omdat template-reuse betekent dat veel code **afhankelijk is van dezelfde templates**, niet opnieuw gegenereerd.
...
Elke run wordt gedocumenteerd in een **spreadsheet** (CSV/Excel):
...
\noindent
Primair format: **tabel** met structuur:
...
\subsection{Externe variabelen \& controllebeperkingen}
...
\begin{enumerate}
  ...
  \item \textbf{Gefocust:} Vier kernmetriekken (tokens, hallucinaties, tijd, kwaliteit).
